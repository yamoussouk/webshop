<template>
  <div class="main-content">
    <div class="container-fluid mt--6">
      <div class="row">
        <div class="col-xl-12">
          <div v-for="product in logos" :key="product.id" class="card">
            <div class="card mb-4">
...
export default {
  ...
  methods: {
    ...
    freezeSource (id) {
      return this.logos.find(pr => parseInt(pr.id) === parseInt(id)).enabled ? '/verified-button.png' : '/freeze-button.png'
    },
    productEnable (id) {
      const headers = {
      ...
      axios.get('http://localhost:8083/admin/enable/logo/' + id, { headers: headers }
      ).then(() => {
        this.changeLogoEnabled(id)
      })
    },
    remove (id) {
      const headers = {
      ...
      // eslint-disable-next-line
      axios.get('http://localhost:8083/admin/delete/logo/' + id, { headers: headers }
      ).then(() => {
        this.removeLogo(id)
        this.tempId = ''
