<template>
  <div class="main-content">
    <div class="container-fluid mt--6">
      <div class="row">
        <div class="col-xl-12">
          <div v-for="product in planners" :key="product.id" class="card">
            <div class="card mb-4">
...
export default {
  ...
  methods: {
    ...mapActions(['changePlannerEnabled', 'removePlanner']),
    freezeSource (id) {
      return this.planners.find(pr => parseInt(pr.id) === parseInt(id)).enabled ? '/verified-button.png' : '/freeze-button.png'
    },
    productEnable (id) {
      const headers = {
      ...
      axios.get('http://localhost:8083/admin/enable/planner/' + id, { headers: headers }
      ).then(() => {
        this.changePlannerEnabled(id)
      })
    },
    remove (id) {
      const headers = {
      ...
      // eslint-disable-next-line
      axios.get('http://localhost:8083/admin/delete/planner/' + id, { headers: headers }
      ).then(() => {
        this.removePlanner(id)
        this.tempId = ''
